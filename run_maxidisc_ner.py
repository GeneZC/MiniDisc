...
def parse_args():
    parser = argparse.ArgumentParser(description="Distil a transformers model with a manual schedule on a named entity recognition task.")
    parser.add_argument(
        "--model_type",
        type=str,
        required=True,
    ...
    parser.add_argument("--layer_or_sparsity_path", type=str, default="6,4L", help="Layer or sparsity path to the student.")
...
def main():
    ...
    args.output_dir = os.path.join(args.output_dir, f"{args.model_type}_{args.model_suffix}_{args.layer_or_sparsity_path}_{args.task_name}")
    ...
    if args.do_train:
        ...
        layer_or_sparsity_key = args.layer_or_sparsity_path[-1]
        layer_or_sparsity_val_path = args.layer_or_sparsity_path[:-1].split(",")
        num_iterations = len(layer_or_sparsity_val_path)
        
        for iteration in range(1, num_iterations + 1):
            current_path = ",".join(layer_or_sparsity_val_path[:iteration]) + "*"
            found_dirs = glob.glob(os.path.join(orig_output_dir, f"{args.model_type}_{args.model_suffix}_{current_path}_{args.task_name}/ckpt_iter_{iteration}"))
            if found_dirs:    
                teacher_model_path = found_dirs[0]
                logger.info(f"***** Found cache {teacher_model_path} for iteration {iteration} *****")
                continue
            
            t_config = config_class.from_pretrained(teacher_model_path)
            #assert t_config.num_hidden_layers % int(layer_or_sparsity_val_path[iteration - 1]) == 0
            add_kwargs_to_config(
                t_config,
                num_relation_heads=args.num_relation_heads,
                num_labels=num_labels,
            #    layer_skip=int(t_config.num_hidden_layers / int(layer_or_sparsity_val_path[iteration - 1])),
            )
            ...
            # Inherit the student from the current teacher.
            ...
            if layer_or_sparsity_key == "L":
                if "enct5" in args.model_type:
                    add_kwargs_to_config(
                        ...
                        num_relation_heads=args.num_relation_heads,
                        num_labels=num_labels,
                        num_layers=int(layer_or_sparsity_val_path[iteration - 1]),
                    #    layer_skip=1,
                    )
                else:
                    add_kwargs_to_config(
                        ...
                        num_relation_heads=args.num_relation_heads,
                        num_labels=num_labels,
                        num_hidden_layers=int(layer_or_sparsity_val_path[iteration - 1]),
                    #    layer_skip=1,
                    )
                ...
                if "enct5" in args.model_type:
                    ...
                    enc_model.sparsify(layer_or_sparsity_val_path[iteration - 1])
                    enc_model.densify()
                    dec_model = getattr(s_model, "decoder", s_model)
                    dec_model.sparsify(layer_or_sparsity_val_path[iteration - 1])
                    dec_model.densify()
                else:
                    base_model = getattr(s_model, s_model.base_model_prefix, s_model)
                    base_model.sparsify(layer_or_sparsity_val_path[iteration - 1])
                    base_model.densify()
                s_config.sparsity = layer_or_sparsity_val_path[iteration - 1]
                truncated_sparsity_map = {s: s_config.sparsity_map[s] for s in s_config.sparsity_map if int(s) >= int(layer_or_sparsity_val_path[iteration - 1])}
