...
def parse_args():
    parser = argparse.ArgumentParser(description="Distil a transformers model with an automatic schedule on a classification task.")
    parser.add_argument(
        "--model_type",
        type=str,
        required=True,
    ...
    # parser.add_argument("--eta", type=int, default=7, help="Eta for sampling.")
...
def main():
    ...
    if args.do_train:
        ...
        for iteration in range(1, args.target_iteration + 1):
            ...
            all_sparsities = [s for s in s_config.sparsity_map]
            sandwich_sparsities = all_sparsities
            # other_sparsities = all_sparsities[1:-3]
            # k = min(len(other_sparsities), args.eta)
            # sandwich_sparsities = [all_sparsities[0], all_sparsities[-3], all_sparsities[-2], all_sparsities[-1]] + other_sparsities[::len(other_sparsities)//k]
            ...
            best_dev_metric = 0.
            best_dev_sparsity = "0"
            best_dev_metric_per_sparsity = {s: {} for s in sandwich_sparsities}
            ...
            for epoch in range(args.num_train_epochs):
                for step, batch in enumerate(train_loader):
                    ...
                    for sparsity in sandwich_sparsities:
                        ...
                        if is_fp16:
                            with amp.autocast():
                                ...
                                loss = model_class.loss_fn(t_output, s_output) / len(sandwich_sparsities)
                        else:
                            s_output = s_model(batch)
                            loss = model_class.loss_fn(t_output, s_output) / len(sandwich_sparsities)
                        loss_item += loss.item()
                    ...
                    if num_completed_steps % args.log_interval == 0:
                        ...
                        s_model.eval()
                        preds, labels = {s: [] for s in sandwich_sparsities}, {s: [] for s in sandwich_sparsities}
                        with torch.no_grad():
