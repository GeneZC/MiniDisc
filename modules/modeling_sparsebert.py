...
class SparseBertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = SparseLinear(config.hidden_size, self.all_head_size, self.attention_head_size, sparse_dim=1)
        self.key = SparseLinear(config.hidden_size, self.all_head_size, self.attention_head_size, sparse_dim=1)
        self.value = SparseLinear(config.hidden_size, self.all_head_size, self.attention_head_size, sparse_dim=1)
    ...
    def attention(self, query, key, value, attention_mask, head_mask):
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size_sparsified,)
